...
st.markdown("입력하신 메모를 바탕으로, 교사의 전문적인 관찰이 담긴 자연스러운 복문 형태의 알림장을 완성해 드립니다.")
...
if submitted:
    if not play_input.strip() and not routine_input.strip():
        ...
        # 입력된 메모를 부드러운 서술형 복문으로 녹여내는 변환 로직
        raw_play = play_input.strip().replace("\n", " ")
        raw_routine = routine_input.strip().replace("\n", " ")
        
        result = (
            f"계절의 향기가 싱그럽게 느껴지는 따뜻한 하루였습니다. 오늘 우리 {name_input}는 "
            f"원에서의 하루를 시작하며 교사와 반갑게 눈을 맞추고 즐거운 놀이를 이어갔답니다. "
            f"특히 오늘 놀이 시간 중 {raw_play} 등의 활동이 이루어졌는데, 놀이 과정에서 스스로 즐거움을 찾아가며 또래와 자연스럽게 어우러지는 모습에서 {name_input}의 성장이 매일매일 느껴져 참 뿌듯했답니다. "
        ...
        result += (
            ...
            f"내일도 우리 {name_input}와(과) 함께 더 많이 웃고 행복한 배움을 만들어갈 수 있도록 따뜻한 시선으로 세심하게 지원하겠습니다. "
            f"오늘 하루 {name_input}가 보여준 반짝이는 모습들을 기억하며, 보호자님께서도 {name_input}와(과) 함께 편안하고 행복한 저녁 시간 보내시기를 바랍니다. "
